...
function statusEnhance() {
  ...
  // 自己主页的 #avatar a 指向 /settings 且 title 是「更新头像」，用户名需从 #info h1 取
  const username = document.querySelector('#info h1')?.textContent?.trim() ?? userInfo.title.trim()

  document.querySelectorAll<HTMLUListElement>('.message li').forEach((li) => {
    // 对话页等场景服务器已输出头像和作者，跳过避免重复
    if (li.querySelector('.avatar'))
      return
